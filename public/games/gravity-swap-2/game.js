...
class Game {
    constructor() {
        ...
        this.minSpawnInterval = 1000; // Fastest speed
    ...
    updateScore(points) {
        ...
        // Difficulty scaling: reduce interval by 10ms for every 10 points
        const scaling = Math.floor(this.score / 10) * 10;
        this.spawnInterval = Math.max(this.minSpawnInterval, 3000 - scaling);
    }
    ...
    resetGame() {
        ...
        // Update button text
        const btn = document.getElementById('actionBtn');
        if (btn) btn.innerText = "SURVIVE!"; // Just a placeholder, button not really used for spamming anymore
    ...
    inputHandler() {
        ...
        // Action Button - Only for Restart now
        const btn = document.getElementById('actionBtn');
        if (btn) {
    ...
    handleGravityChange(x, y) {
        ...
        if (changed) {
            ...
            // Reset timer a bit? Maybe not on gravity change, makes it too easy to stall.
        }
    }
